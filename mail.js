...
async function sendInventory(inventory) {
  ...
  html += `<h4 style="background:rgba(0, 128, 0,0.033);text-align:center"> Powered by ShopTorama<sup>&#174;</sup> - All rights reserved. &#169; ${fullDate}}</p> </body></html>`;
...
async function sendExpense(expense, userId = null) {
  ...
  if (expenseAccount || expenseAccount !== 'undefined') {
    expenseAccount = `<p> Expense Acount: ${expenseAccount} </p>`;
  } else {
    expenseAccount = '';
  }
  let balance = 0;
  ...
  <p>Category: ${category}</p>  ${expenseAccount}`;
...
async function sendProduceexpense(expense, userId = null) {
  ...
  // some content
  let expenseId = expense.produceexpense_id;
  let memo;
  let remarks;
...
async function sendQaNote(note, item) {
  ...
  html += `<h4 style="background:rgba(0, 128, 0,0.033);text-align:center"> Powered by ShopTorama<sup>&#174;</sup> - All rights reserved.<sup>&#169;</sup> ${fullDate}</p> </body></html>`;
...
async function sendCashdeposit(item, user) {
  try {
    ...
    let subject;
    if (item.status === "SEEN") {
      subject = `Cash Deposit ${item.status}`;
    } else {
...
async function sendCallActivity(report, user) {
  try {
    let format1 = "DD-MM-YYYY hh:mm:ss";
    let date;
    ...
    let subject = `${caller} called from ${callerPhone} on ${date}`;
    ...
    let html = `<!DOCTYPE html><html><body style="text-align:center;"><p style="background:rgba(0, 128, 0,0.051); text-align:center;">
                ${date}</p>
                <p style="margin:0px;"> Creator ${titleCase(user?.name)}</p>
                <p style="margin:0px;"><b> Caller:</b> ${caller}</p> 
                <p style="margin:0px;"><b> Caller Phone:</b> ${callerPhone}</p>
                <p style="margin:0px;"> <b>Message:</b> </p> <p>${message}</p><hr>
                <p style="margin:0px;"> <b>Receiver Name:</b> ${receiverName}</p>
                <p style="margin:0px;"> <b>Receiver Phone:</b><span>${receiverPhone}</span></p> 
                <p style="margin:0px;"> <b>Response:</b> </p> <span>${response}</span>
                <p style="margin:0px;"> <b>Site:</b> <span>${site}</span><hr></p>
                `;
...
async function sendPeopleMail(record) {
