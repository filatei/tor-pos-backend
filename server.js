...
io.sockets.on('connection', async (socket) =>  {
  ...
  socket.on('getStock',  async (from, msg)  =>  {
    ...
    inventory = await Inventory.find( {store: from.store} ).sort({ createdAt:-1 }).populate('name').populate('sender').populate('receiver');
   ...
   socket.on('getExpenses',  async (from, msg)  =>  {
    ...
    socket.on('newNote', function (from, msg) {
      console.log(`Note ${from.note} made on expense id # ${from.expenseId}`, ' saying ', msg);
