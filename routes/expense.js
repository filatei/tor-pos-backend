...
async function sendMail(expense) {
 ...
 let toEmail = expense.vendor.email || 'expenses@torama.ng';
 expenseId = expenseId.toString().padStart(5, '0')
 let subject = `ShopTorama Expense Confirmation for (# ${expenseId})`
 let curr =  process.env.naira
 let total = expense.qty * expense.rate;
 let status = expense.status;
 let location = expense.site;
 
 let date = (expense.createdAt).toString() || new Date().toString();
...
//  product += `</tbody><tfoot><tr><td colspan="4" style="text-align:right;" > Sum: ${derived_total} ${curr}</td></tr></tfoot></table>`;
 ...
 let html = `<!DOCTYPE html><html><body style="text-align:center;"><img src="${logo}" alt="logoimg" width="50"><p style="background:rgba(0, 128, 0,0.051); text-align:center;">${date}</p><h2>EXPENSE CONFIRMED</h2><p> Hi ${vendor},</p>`;
 html += `<p>We received your Expense Request # ${expenseId} for ${curr} ${total.toLocaleString()} </p> <p>Factory Location: ${location}</p>`;
 html += `${item}`;
...
router.get('',(req, res, next) => {
  ...
  const expenseQuery = Expense.find().sort({createdAt:-1}).populate('vendor').populate('name');
