...
router.put("/:id", checkAuth, (req, res, next) => {
  ...
  isOpen().then(sm => {
    console.log(mailStat, 'mailstat sent')
  })
  .catch(err => {
...
router.get("/:id", (req, res, next) => {
  ...
  router.put('/notes/:id', checkAuth, Utils.upload2.any(), function (req, res, next) {
    ...
    if (req.files) {
      ...
      req.files.forEach(file => {
          if (file.originalname == 'blob') {
            fileName = 'uploads/expenses/'  + req.userData.userId + '/' + file.filename 
          } else {
            fileName = 'uploads/expenses/'  + req.userData.userId + '/' + file.filename
          }
